...
#define M_BLOCKING 384
#define N_BLOCKING 192
#define K_BLOCKING 2048

#define KERNEL_14_8x8_intrinsics \
...
void macro_kernel_k14(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double *C, int LDC){
    int i, j, k;
        ...
        k_inc = (K-k_count>K_BLOCKING)?K_BLOCKING:K-k_count;
            for(m_count=0;m_count<M;m_count+=m_inc){
                ...
                for(n_count=0; n_count<N; n_count+=n_inc){
                    ...
                    macro_kernel_k14(m_inc, n_inc, k_inc, alpha, &A(m_count,k_count), LDA,  &B(k_count,n_count), LDB, &C(m_count, n_count), LDC);
            }
        }
    }
