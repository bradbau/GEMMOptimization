...
#define KERNEL_12_8x8_intrinsics \
...
void macro_kernel_k12(int M, int N, int K, double alpha, double *A, int LDA, double *B, int LDB, double *C, int LDC){
    int i, j, k;
    ...
    for(n_count=0; n_count<N; n_count+=n_inc){
        ...
        for(k_count=0;k_count<K;k_count+=k_inc){
            ...
            for(m_count=0;m_count<M;m_count+=m_inc){
                ...
                macro_kernel_k12(m_inc, n_inc, k_inc, alpha, &A(m_count,k_count), LDA,  &B(k_count,n_count), LDB, &C(m_count, n_count), LDC);
            }
        }
    }
